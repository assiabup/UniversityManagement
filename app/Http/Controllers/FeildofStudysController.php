...
class FeildofStudysController extends Controller
{
            ...
            try {
                ...
                foreach ($data[0] as $row) {
                    ...
                    if ($student) {
                        ...
                        // Associez la notification à l'utilisateur
    ...
    public function show8(Request $request)
    {
        ...
        foreach ($students as $student) {
            ...
            // Logique conditionnelle pour déterminer le statut de validation
            if ($fieldOfStudy->name === 'cycle preparatoire') {
                if ($rattrapageStudent && $rattrapageStudent->score >= 10) {
                    $scoreToSave = 10;
                } elseif ($rattrapageStudent && $rattrapageStudent->score < 10) {
                    $scoreToSave = max($grade->score, $rattrapageStudent->score);
                } else {
                    $scoreToSave = $grade->score;
                }
                $student->validated = $scoreToSave >= 10;
            } else {
                if ($rattrapageStudent && $rattrapageStudent->score >= 12) {
                    $scoreToSave = 12;
                } elseif ($rattrapageStudent && $rattrapageStudent->score < 12) {
                    $scoreToSave = max($grade->score, $rattrapageStudent->score);
                } else {
                    $scoreToSave = $grade->score;
                }
                $student->validated = $scoreToSave >= 12;
            }
        }
    ...
    {
        // Logique pour retourner le score minimum en fonction de l'ID de la filière
        if ($fieldOfStudyId === 'cycle preparatoire') {
            return 10;
        } else {
            return 12;
        }
