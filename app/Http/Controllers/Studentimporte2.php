...
class Studentimporte2 extends Controller
{
    ...
    {
        // Supprimer l'enregistrement lié dans la table users
        $student = Student::findOrFail($id);
        $userId = $student->user_id; // Supposons que user_id est la clé étrangère qui fait référence à la table users
        User::where('id', $userId)->delete();

        // Supprimer les enregistrements liés dans la table rattrapage_students
        RattrapageStudent::where('student_id', $id)->delete();
